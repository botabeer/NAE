...
# --- حفظ حالة المستخدم الحالي للألغاز ---
user_current_riddle = {}  # user_id: {"idx":0, "state":"question"}
user_riddle_index = {}    # user_id: مؤشر التكرار لكل مستخدم
...
# --- متابعة حالة كل مستخدم للألعاب ---
user_game_state = {}  # user_id: {"game_index": 0, "question_index": 0, "answers": []}

# --- تحميل نصوص التحليل المفصل لكل لعبة ---
try:
...
def home():
    return "بوت الألغاز والألعاب يعمل", 200
...
def handle_message(event):
    ...
    if command:
        if command == "سؤال":
            ...
            riddle = riddles[idx]
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text=f"السؤال: {riddle.get('question','')}"))
    ...
    # --- تلميح أو لمح للألغاز ---
    if text in ["تلميح", "لمح"]:
        if user_id in user_current_riddle:
            idx = user_current_riddle[user_id]["idx"]
            hint = riddles[idx].get("hint", "لا يوجد تلميح متاح لهذا اللغز.")
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text=f"تلميح: {hint}"))
            user_current_riddle[user_id]["state"] = "hint"
        else:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text="لم يتم اختيار لغز بعد."))
        return

    # --- إجابة أو جاوب للألغاز ---
    if text in ["جاوب", "الإجابة"]:
        if user_id in user_current_riddle:
            idx = user_current_riddle[user_id]["idx"]
            answer = riddles[idx].get("answer", "لا توجد إجابة لهذا اللغز.")
    ...
    if text == "لعبه":
        ...
        description = (
            "لعبة تحليل الشخصية:\n"
            "- ستُطرح عليك عدة أسئلة لكل لعبة.\n"
            "- اختر الإجابة التي تعبر عنك أكثر (أ، ب، ج).\n"
            "- بعد الإجابة على كل الأسئلة، ستتلقى تحليل مفصل لشخصيتك.\n"
            "- يمكنك إعادة اللعب مرة أخرى لاختبار جوانب مختلفة من شخصيتك."
        )
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=f"{games_titles}\n\n{description}"))
        return
